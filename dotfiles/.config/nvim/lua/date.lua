        ...
        repeat -- print(sw:aimchr())
            ...
            elseif sw("^(%a+)[/\\%s,-]?%s*") then --print("$Words")
                ...
                elseif not inlist(x, sl_weekdays, 2, sw) then
                    sw:back()
                    -- am pm bce ad ce bc
                    if sw("^([bB])%s*(%.?)%s*[Cc]%s*(%2)%s*[Ee]%s*(%2)%s*")
                        or sw("^([bB])%s*(%.?)%s*[Cc]%s*(%2)%s*")
                    then
                        e = e and error_dup() or -1
                    elseif sw("^([aA])%s*(%.?)%s*[Dd]%s*(%2)%s*")
...
local function date_from(arg1, arg2, arg3, arg4, arg5, arg6, arg7)
    ...
    local h, r, s, t =
    tonumber(arg4 or 0), tonumber(arg5 or 0), tonumber(arg6 or 0), tonumber(arg7 or 0)
...
local tvspec = {
    ...
    ["%:z"] = function(self)
        local b = -self:getbias()
        local x = abs(b)
        return fmt("%s%.2d:%.2d", b < 0 and "-" or "+", fix(x / 60), floor(mod(x, 60)))
    end,
...
function dobj:fmt0(str)
    local single_sub = gsub(str, "%%[%a%%\b\f]", function(x)
        local f = tvspec[x]
        return (f and f(self)) or x
    end)

    local double_sub = gsub(single_sub, "%%[%a:][%a]", function(x)
        print(x)
        local f = tvspec[x]
        return (f and f(self)) or x
    end)
